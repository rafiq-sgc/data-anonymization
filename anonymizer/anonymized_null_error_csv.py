...
from csv_to_parquet_big import CsvToParquetBig
...
def data_anonymization(file_dir, out_dir, check_dir=None):
    ...
    for file in dir_list:
        ...
        data = read_csv_with_error(file_path)
        ...
        if data:
            ...
            if anonymize:
                ...
                write_dicts_to_csv(out_file, data)
                csv_to_parquet = CsvToParquetBig()
                # out_file = Path(out_dir) / file
                dest_dir = Path('/media/zaman/Data Storage/anonymization/data_anonymization_new/csv_to_parquet_40/parquet_tables')
                csv_to_parquet.csv_to_parquet_big(file_path, dest_dir, dest_dir)
...
def main(filenames):
    ...
    large_file_path = '/media/zaman/Data Storage/anonymization/data_anonymization_new/data_big/stud_hours_crs/STUD_HOURS_CRS_.csv'
    save_dir = '/media/zaman/Data Storage/anonymization/data_anonymization_new/data_big/splitted_stud_hours_crs'
    # split_file = split_large_csv_file(large_file_path, save_dir)

    file_dir = '/media/zaman/Data Storage/anonymization/data_anonymization_new/others_tables' # running all for this directory, 7 big file are remaining
